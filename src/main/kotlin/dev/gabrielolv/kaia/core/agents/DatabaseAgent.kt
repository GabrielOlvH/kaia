...
import dev.gabrielolv.kaia.utils.validateAndAnalyzeSelectQuery
...
fun Agent.Companion.withDatabaseAccess(
    ...
    **Instructions:**
    ...
    2.  Determine the appropriate SQL query structure (`SELECT` only) and the specific parameter values derived from the user's request.
    ...
    **Database Schema:**
    ...
    builder.processor = processor@{
        flow {
            ...
            val responses = provider.generate(it.content, options).toList()
            ...
            } ?: return@flow emit(LLMMessage.SystemMessage("There was an error while executing the query."))
            val resultsAsString =
                rows.joinToString("\n") { entry -> entry.entries.joinToString { "${it.key}=${it.value}" } }
            emit(LLMMessage.SystemMessage("Query Results:\n$resultsAsString"))
        }
