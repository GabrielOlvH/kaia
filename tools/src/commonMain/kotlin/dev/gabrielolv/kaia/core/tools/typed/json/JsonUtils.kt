...
expect fun JsonUtils.handleSpecialPrimitive(element: JsonPrimitive, type: KClass<*>?): Any?

expect fun JsonUtils.handleSpecialTypes(value: Any?): JsonElement

expect fun JsonUtils.handleEnumSchema(builder: JsonObjectBuilder, type: KClass<*>?)
...
object JsonUtils {
    ...
    private fun parsePrimitive(element: JsonElement, type: KClass<*>?): Any? {
        ...
        return when (type) {
            ...
            else -> handleSpecialPrimitive(element, type)
        }
    }
        ...
        put("items", buildJsonObject {
            if (prop.isComplexList) {
                ...
                when (prop.itemType) {
                    ...
                    else -> handleEnumSchema(this, prop.itemType)
                }
            }
        })
    }
    ...
    private fun JsonObjectBuilder.buildPrimitiveSchema(prop: Property<*>) {
        when (prop.type) {
            ...
            Float::class, Double::class -> put("type", "number")
            Boolean::class -> put("type", "boolean")
            else -> handleEnumSchema(this, prop.type)
        }
    }
